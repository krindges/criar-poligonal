...
cursor_style = """
<style>
    /* Força cursor padrão (seta) no mapa inteiro */
    .leaflet-container, .leaflet-pane, .leaflet-overlay-pane, 
    .leaflet-shadow-pane, .leaflet-tile-pane, .leaflet-marker-pane,
    .leaflet-tooltip-pane, .leaflet-popup-pane, .leaflet-map-pane {
        cursor: default !important;
    }
    
    /* Impede mudança para "grab" ao passar o mouse */
    .leaflet-container.leaflet-grab,
    .leaflet-container.leaflet-dragging {
        cursor: default !important;
    }

    /* Remove o cursor "pointer" dos marcadores (se quiser clicar ainda, não ative essa parte) */
    .leaflet-interactive {
        cursor: default !important;
    }
</style>
...
cursor_element = MacroElement()
cursor_element._template = Template(cursor_style)
mapa.get_root().add_child(cursor_element)
